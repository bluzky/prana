defmodule Prana.Integrations.Wait do
  ...
  def wait(params) do
    ...
    case mode do
      ...
      nil -> {:error, Error.new("config_error", "mode is required")}
      _ -> {:error, Error.new("config_error", "mode must be 'interval', 'schedule', or 'webhook'")}
    end
  end
  ...
  def wait_interval(params) do
    ...
    with :ok <- validate_duration(duration),
      ...
      {:error, reason} ->
        {:error, Error.new("interval_config_error", reason)}
    end
  end
  ...
  def wait_schedule(params) do
    ...
    with :ok <- validate_schedule_at(schedule_at),
      ...
      {:error, reason} ->
        {:error, Error.new("schedule_config_error", reason)}
    end
  end
  ...
  def wait_webhook(params) do
    ...
    case validate_timeout_hours(timeout_hours) do
      ...
      {:error, reason} ->
        {:error, Error.new("webhook_config_error", reason)}
    end
  end
