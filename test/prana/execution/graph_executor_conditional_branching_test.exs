defmodule Prana.Execution.ConditionalBranchingTest do
  ...
  describe "conditional workflow completion" do
    test "workflow completes when no more ready nodes exist (if/else)" do
      workflow = create_basic_if_else_workflow()
      {:ok, execution_graph} = WorkflowCompiler.compile(workflow, "start")
      ...
      # Should be complete - adult path executed, minor path not needed
      assert GraphExecutor.workflow_complete?(execution, execution_graph)
    end

    test "workflow not complete when ready nodes exist" do
      workflow = create_basic_if_else_workflow()
      {:ok, execution_graph} = WorkflowCompiler.compile(workflow, "start")
      ...
      # Should NOT be complete - condition node still ready
      refute GraphExecutor.workflow_complete?(execution, execution_graph)
    end
  end
  ...
  describe "edge cases and error handling" do
    ...
    test "workflow completes correctly with failed conditional node" do
      ...
      # Should be complete - no more ready nodes after failure
      assert GraphExecutor.workflow_complete?(execution, execution_graph)
    end

    test "empty workflow completes immediately" do
      ...
      # This should handle gracefully even though it's not a realistic scenario
      execution = %Execution{
        id: "empty_execution",
      ...
      # Can't compile empty workflow, but test completion logic
      mock_execution_graph = %ExecutionGraph{
      ...
      # Empty workflow should be complete
      assert GraphExecutor.workflow_complete?(execution, mock_execution_graph)
    end
  end
