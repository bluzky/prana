defmodule PranaDemo.WorkflowRunner do
  ...
  def execute_workflow(workflow, input, opts \\ []) do
    ...
    with {:ok, execution_graph} <- build_graph(workflow),
      ...
      case GraphExecutor.execute_workflow(persisted_execution) do
        {:ok, result} -> handle_result(result)
        {:suspend, execution} -> handle_result({:suspend, execution})
  ...
  defp init_execution(execution_graph, input, opts) do
    ...
    enhanced_execution = %{execution_with_runtime | 
      metadata: Map.merge(execution_with_runtime.metadata, %{
  ...
  defp handle_result(result) do
    case result do
      ...
      # Handle case where GraphExecutor returns execution directly
      %Prana.WorkflowExecution{status: :completed} = execution ->
        Logger.info("Workflow completed successfully")
        update_execution_db(execution, :completed)
        {:ok, execution}
        
      %Prana.WorkflowExecution{status: :suspended} = execution ->
        Logger.info("Workflow suspended")
        ETSStorage.update_execution(execution)
        handle_suspend(execution)
        
      %Prana.WorkflowExecution{status: :failed} = execution ->
        Logger.error("Workflow execution failed")
        update_execution_db(execution, :failed)
        {:error, execution}
    end
