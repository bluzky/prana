defmodule PranaDemo.DemoWorkflow do
  ...
  def create_simple_workflow do
    ...
    set_data_node = Node.new("Set Data", "manual", "set_data", %{
      "data" => %{
    ...
    process_node = Node.new("Process Adult", "manual", "process_adult", %{
  ...
  def create_conditional_workflow do
    # Create workflow
    workflow = Workflow.new("Conditional Demo Workflow", "A conditional workflow for demonstration")
    ...
    condition_node = Node.new("Age Check", "logic", "if_condition", %{
      "condition" => "$nodes.set_data.data.age >= 18"
    }, "age_check")
    
    process_adult_node = Node.new("Process Adult", "manual", "process_adult", %{
      "user_data" => "$nodes.set_data.data"
    }, "process_adult")
    
    process_minor_node = Node.new("Process Minor", "manual", "process_minor", %{
      "user_data" => "$nodes.set_data.data"
    }, "process_minor")
    ...
    # Add connections
    workflow = Enum.reduce(connections, workflow, fn conn, acc_workflow ->
  ...
  def create_loop_workflow do
    ...
    attempt_node = Node.new("Attempt Operation", "manual", "attempt_operation", %{
      "max_attempts" => 3,
      "data" => "$input"
    }, "attempt_operation")
    
    # Check if retry is needed (attempt_count < max_attempts)
    retry_check_node = Node.new("Check Retry", "logic", "if_condition", %{
      "condition" => "$nodes.attempt_operation.attempt_count < $nodes.attempt_operation.max_attempts"
    }, "retry_check")
    
    increment_node = Node.new("Increment Retry", "manual", "increment_retry", %{
      "attempt_count" => "$nodes.attempt_operation.attempt_count"
    }, "increment_retry")
    
    process_success_node = Node.new("Process Success", "manual", "process_adult", %{
    ...
    connections = [
      %Connection{from: "trigger", from_port: "success", to: "attempt_operation", to_port: "input"},
      %Connection{from: "attempt_operation", from_port: "success", to: "process_success", to_port: "input"},
      %Connection{from: "attempt_operation", from_port: "error", to: "retry_check", to_port: "input"},
      %Connection{from: "retry_check", from_port: "true", to: "increment_retry", to_port: "input"},
      %Connection{from: "retry_check", from_port: "false", to: "process_success", to_port: "input"},
      %Connection{from: "increment_retry", from_port: "success", to: "attempt_operation", to_port: "input"}
    ]
  ...
  def create_sub_workflow_demo(execution_mode \\ "fire_and_forget") do
    ...
    main_workflow = Workflow.new("Sub Workflow Demo - #{execution_mode}", "Demonstrates sub-workflow execution patterns")
    ...
    sub_workflow_node = Node.new("Execute Sub Workflow", "workflow", "execute_workflow", %{
    ...
    process_result_node = Node.new("Process Result", "manual", "process_adult", %{
    ...
    # Create connections
    connections = [
      %Connection{from: "trigger", from_port: "success", to: "execute_sub_workflow", to_port: "input"},
      %Connection{from: "execute_sub_workflow", from_port: "success", to: "process_result", to_port: "input"}
    ]
    
    # Add connections
    main_workflow = Enum.reduce(connections, main_workflow, fn conn, acc_workflow ->
      {:ok, updated_workflow} = Workflow.add_connection(acc_workflow, conn)
      updated_workflow
    end)
  ...
  defp create_simple_sub_workflow do
    ...
    process_node = Node.new("Sub Process", "manual", "set_data", %{
      "result" => %{
  ...
  def create_wait_demo do
    ...
    wait_node = Node.new("Wait Timer", "wait", "wait", %{
      "mode" => "interval",
      "delay_ms" => 2000  # 2 second delay
    }, "wait_timer")
    
    process_after_wait_node = Node.new("Process After Wait", "manual", "process_adult", %{
    ...
    connections = [
      ...
      %Connection{from: "wait_timer", from_port: "success", to: "process_after_wait", to_port: "input"}
    ]
  ...
  def run_sub_workflow_demo(execution_mode \\ "fire_and_forget") do
    ...
    case WorkflowRunner.execute_workflow(main_workflow, input_data, %{}) do
      ...
      {:error, {:suspend, execution}} ->
        Logger.info("Sub workflow demo (#{execution_mode}) suspended as expected - GraphExecutor returned suspension!")
  ...
  def run_all_sub_workflow_demos do
    ...
    results = Enum.map(execution_modes, fn mode ->
